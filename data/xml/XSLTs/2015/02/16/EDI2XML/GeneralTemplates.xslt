		...
		<xsl:if test="$AFSSPEC!='' ">
			<MedicalSpecialityCode>
				<xsl:choose>
					...
					<xsl:when test=" $AFSSPEC='00' ">blandet</xsl:when>
					<xsl:when test=" $AFSSPEC='14' ">palliativ</xsl:when>
					<xsl:when test=" $AFSSPEC='15' ">akut</xsl:when>
					<xsl:when test=" $AFSSPEC='39' ">sexologi</xsl:when>
					<xsl:when test=" $AFSSPEC='80' ">paediatri_sygehus</xsl:when>
					<xsl:when test=" $AFSSPEC='92' ">retsmedicin</xsl:when>
					<xsl:when test=" $AFSSPEC='98' ">fysioterapi_sygehus</xsl:when>
					<xsl:when test=" $AFSSPEC='0581' ">almenlaege_vagtkoersel</xsl:when>
					<xsl:when test=" $AFSSPEC='0582' ">almenlaege_vagtlaegehjaelp</xsl:when>
					<xsl:when test=" $AFSSPEC='1080' ">vagtlaege</xsl:when>
					<xsl:when test=" $AFSSPEC='1083' ">vagtlaegehjaelp</xsl:when>
					<xsl:when test=" $AFSSPEC='1082' ">vagtlaegehjaelp_kbh</xsl:when>
					<xsl:when test=" $AFSSPEC='2517' ">neurokirurgi_praksis</xsl:when>
					<xsl:when test=" $AFSSPEC='4049' ">tandplejere</xsl:when>
					<xsl:when test=" $AFSSPEC='4557' ">ridefysioterapi</xsl:when>
					<xsl:when test=" $AFSSPEC='4562' ">fysioterapi_vederlagsfri</xsl:when>
					<xsl:when test=" $AFSSPEC='4565' ">ridefysioterapi_vederlagsfri</xsl:when>
					<xsl:when test=" $AFSSPEC='4658' ">teddy</xsl:when>
					<xsl:when test=" $AFSSPEC='5064' ">kiropraktor_64</xsl:when>
					<xsl:when test=" $AFSSPEC='6059' ">fodterapi_radioaktiv</xsl:when>
					<xsl:when test=" $AFSSPEC='6060' ">fodterapi_leddegigt</xsl:when>
					<xsl:when test=" $AFSSPEC='7044' ">med_laboratorier_kpll</xsl:when>
					<xsl:when test=" $AFSSPEC='7048' ">med_laboratorier_ssi</xsl:when>
					<xsl:otherwise>
