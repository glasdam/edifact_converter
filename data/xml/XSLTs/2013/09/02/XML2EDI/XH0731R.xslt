<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/medcom.dk/xml/schemas/2013/09/02/" version="1.0">
	...
	<xsl:template name="createFysioterapiHenvisningDianoseText">
		...
		<xsl:choose>
		  ...
		  <xsl:when test=" $DC='2A' and $DT='Følger efter hjerte-, rygmarvs- og hjernehindebetændelse'"/>
			...
			<xsl:when test=" $DC='2O' and $DT='Syringomyeli'"/>
		    <xsl:when test=" $DC='511' and $DT='Alm fysioterapi'"/>
		    <xsl:when test=" $DC='621' and $DT='Vederlagsfri fysioterapi'"/>
		    <xsl:when test=" $DC='624' and $DT='Vederlagsfri fysioterapi, fortsat behandling'"/>
		    <xsl:when test=" $DC='571' and $DT='Ridefysioterapi'"/>
		    <xsl:when test=" $DC='651' and $DT='Vederlagsfri ridefysioterapi'"/>
		    <xsl:when test=" $DC='654' and $DT='Vederlagsfri ridefysioterapi, fortsat behandling'"/>
			<xsl:otherwise>
