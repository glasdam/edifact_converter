...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2005/08/07/" version="1.0">
	...
	<!-- RADIOLOGYREPORT -->
	<xsl:template match="m:RadiologyReport">
		...
		<xsl:variable name="ExaminationStart" select="m:ExaminationStart"/>
		<xsl:variable name="ExaminationEnd" select="m:ExaminationEnd"/>
		...
		<Brev>
			...
			<BrevIndhold>
				...
				<S11>
					...
					<xsl:if test="count($ExaminationStart)=1">
					<DTM>
						<Elm>
							<SubElm>90</SubElm>
							<SubElm>
								<xsl:variable name="DT" select="$ExaminationStart"/>
								<xsl:variable name="DT203" select="concat(substring($DT/m:Date,1,4),substring($DT/m:Date,6,2),substring($DT/m:Date,9,2),substring($DT/m:Time,1,2),substring($DT/m:Time,4,2))"/>
								<xsl:value-of select="$DT203"/>
							</SubElm>
							<SubElm>203</SubElm>
					...
					<xsl:if test="count($ExaminationEnd)=1">
						<DTM>
							<Elm>
								<SubElm>91</SubElm>
								<SubElm>
									<xsl:variable name="DT" select="$ExaminationEnd"/>
									<xsl:variable name="DT203" select="concat(substring($DT/m:Date,1,4),substring($DT/m:Date,6,2),substring($DT/m:Date,9,2),substring($DT/m:Time,1,2),substring($DT/m:Time,4,2))"/>
									<xsl:value-of select="$DT203"/>
								</SubElm>
								<SubElm>203</SubElm>
					...
					<S14>
						...
						<CIN>
							...
							<Elm>
								...
								<SubElm>
									<xsl:variable name="DTC" select="$Diagnose/m:DiagnoseTypeCode/text()"/>
									<xsl:choose>
										<xsl:when test="$DTC='SKSdiagnosekode' ">SKS</xsl:when>
										<xsl:when test="$DTC='uspecificeretkode' ">USP</xsl:when>
										<xsl:when test="$DTC='ICPCkode' ">ICP</xsl:when>
