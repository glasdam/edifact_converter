...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2005/08/07/" version="1.0">
	...
	<!-- DISCHARGELETTER -->
	<xsl:template match="m:XrayRequest">
		...
		<xsl:variable name="Pregnancy" select="$RelClinInfo/m:FirstDateOfPeriod"/>
			...
			<BrevIndhold>
				...
				<S01>
					...
					<NAD>
						...
						<Elm>
							<xsl:if test="$Participant/m:Identifier">
							<SubElm>
								<xsl:value-of select="$Participant/m:Identifier"/>
							</SubElm>
							<xsl:call-template name="identifierCode2edi">
				...
				<S12 maxOccurs="2">
					...
					<XFTX maxOccurs="9" comparable="false">
						...
						<xsl:if test="count($Pregnancy)=1">
							...
							<Elm>
								<SubElm>
									<xsl:for-each select="$Pregnancy/text()|$Pregnancy/*"><xsl:apply-templates select="."/></xsl:for-each>
								</SubElm>
							</Elm>
						</xsl:if>
