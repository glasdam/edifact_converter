...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2005/08/07/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDDIS' and SubElm[5]='D0233L']]">
		...
		<xsl:variable name="DTMOutPatientFirstVisit" select="$S11/DTM[Elm[1]/SubElm[1]='90']"/>
		<xsl:variable name="DTMOutPatientLastVisit" select="$S11/DTM[Elm[1]/SubElm[1]='91']"/>
		...
		<OutPatientDischargeLetter>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XD0233L</VersionCode>
					<StatisticalCode>XDIS02</StatisticalCode>
					<xsl:for-each select="$Dtm">
						<Authorisation>
							<xsl:call-template name="DTM203ToDateTime"/>
						</Authorisation>
					</xsl:for-each>
					<TypeCode>XDIS02</TypeCode>
					<StatusCode>
			...
			<xsl:for-each select="$DTMOutPatientFirstVisit">
				<OutPatientFirstVisit>
					<xsl:call-template name="DTM203ToDateTime"/>
				</OutPatientFirstVisit>
			</xsl:for-each>
			<xsl:for-each select="$DTMOutPatientLastVisit">
				<OutPatientLastVisit>
					<xsl:call-template name="DTM203ToDateTime"/>
				</OutPatientLastVisit>
			</xsl:for-each>
