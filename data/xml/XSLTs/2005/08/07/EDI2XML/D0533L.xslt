...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2005/08/07/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDDIS' and SubElm[5]='D0533L']]">
		...
		<xsl:variable name="DTMExaminationStart" select="$S11/DTM[Elm[1]/SubElm[1]='90']"/>
		<xsl:variable name="DTMExaminationEnd" select="$S11/DTM[Elm[1]/SubElm[1]='91']"/>
		...
		<RadiologyReport>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XD0533L</VersionCode>
					<StatisticalCode>XDIS05</StatisticalCode>
			...
			<xsl:for-each select="$DTMExaminationStart">
				<ExaminationStart>
					<xsl:variable name="DTM" select="."/>
					<xsl:variable name="DT203" select="$DTM/Elm[1]/SubElm[2]"/>
					<xsl:if test="string-length($DT203)!=12">
			...
			<xsl:for-each select="$DTMExaminationEnd">
				<ExaminationEnd>
					<xsl:variable name="DTM" select="."/>
					<xsl:variable name="DT203" select="$DTM/Elm[1]/SubElm[2]"/>
					<xsl:if test="string-length($DT203)!=12">
