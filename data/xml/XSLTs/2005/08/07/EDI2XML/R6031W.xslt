...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2005/08/07/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDPRE' and SubElm[5]='R6031W']]">
		...
		<xsl:variable name="S01Receiver" select="BrevIndhold/S01[PNA/Elm[1]/SubElm[1]='SSP']"/>
		<xsl:variable name="S01PrefPharmacy" select="BrevIndhold/S01[PNA/Elm[1]/SubElm[1]='SE']"/>
		...
		<xsl:variable name="IkkeBrugteSegmenter" select="	*[not(name()='UNH' or name()='BrevIndhold' or name()='UNT')]
												...
												|	BrevIndhold/S01[not(PNA/Elm[1]/SubElm[1]='PO' or PNA/Elm[1]/SubElm[1]='SSP' or PNA/Elm[1]/SubElm[1]='SE')]"/>
		<xsl:variable name="AnswerTo" select="$S01Sender/COM[position() = 2]"/>
		...
		<PrescriptionRequest>
			<Letter>
				...
				<VersionCode>XR6031W</VersionCode>
				<StatisticalCode>XPRE60</StatisticalCode>
				<xsl:for-each select="$Dtm">
					<Authorisation>
						<xsl:call-template name="DTM204ToDateTimeWithSec"/>
					</Authorisation>
				</xsl:for-each>
				<TypeCode>XPRE60</TypeCode>
			</Letter>
			<xsl:for-each select="$S01Sender">
				<Sender>
					...
					<xsl:for-each select="COM[position() = 1]">
			...
			<xsl:for-each select="$S01Receiver">
				<Receiver>
					...
					<xsl:if test="PNA/Elm[2]/SubElm[1]!='' or PNA/Elm[6]/SubElm[2]!='' or QUA/Elm[2]/SubElm[1]!=''  or  EMP/Elm[2]/SubElm[1]!='' ">
						<Issuer>
							<xsl:for-each select="PNA">
			...
			<xsl:for-each select="$S01PrefPharmacy">
				<PreferredPharmacy>
					<xsl:for-each select="PNA">
			...
			<xsl:for-each select="$S03PatOrRel">
				<PatientOrRelative>
					...
					<EmailStatus>
						<xsl:variable name="ES" select="Elm[1]/SubElm[2]"/> 
						<xsl:choose>
								<xsl:when test="$ES='PEM'">permanent</xsl:when>
								<xsl:when test="$ES='TEM'">midlertidig</xsl:when>
								<xsl:otherwise>
									<xsl:call-template name="ValueListError">
										<xsl:with-param name="ValueNode" select="$ES"/>
										<xsl:with-param name="Text">Ukendt EMAILTYPE</xsl:with-param>
									</xsl:call-template>
								</xsl:otherwise>
							</xsl:choose>

					</EmailStatus>
					<EmailIdentifier><xsl:value-of select="Elm[1]/SubElm[1]"/></EmailIdentifier>
					</xsl:for-each>
			...
			<xsl:for-each select="$S02/INP">
				<xsl:call-template name="Error">
				<xsl:with-param name="Text">INP er ikke tilladt i genbestilling
				</xsl:with-param>
			</xsl:call-template>
