...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/medcom.dk/xml/schemas/2004/06/01/" version="1.0">
	...
	<!-- WARNINGOFDISCHARGE -->
	<xsl:template match="m:WarningOfDischarge">
		<xsl:variable name="Letter" select="m:Letter"/>
		<xsl:variable name="Sender" select="m:Sender"/>
		<xsl:variable name="Signatur" select="$Sender/m:Signatur"/>
		...
		<xsl:variable name="FamDoc" select="m:FamilyDoctor"/>
		<xsl:variable name="Admission" select="m:Admission"/>
		<xsl:variable name="PlanConference" select="m:PlanConference"/>
		<xsl:variable name="EndOfTreatment" select="m:EndOfTreatment"/>				
		<xsl:variable name="PlannedDischarge" select="m:PlannedDischarge"/>
		<xsl:variable name="CareDescription" select="m:CareDescription"/>
		<Brev>
			<UNH>
				<Elm>
					<SubElm>
			...
			<BrevIndhold>
				...
				<xsl:if test="count($FamDoc)=1">
					<S01>
						<xsl:variable name="Participant" select="$FamDoc"/>
						<Elm>
							<SubElm>01</SubElm>
						</Elm>
						<NAD>
						...
						<CON>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="$Participant/m:TelephoneSubscriberIdentifier"/></SubElm>
								<SubElm>TE</SubElm>
							</Elm>
						</CON>
						<SEQ>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="3+count($CCReceiver)"/></SubElm>
							</Elm>
						</SEQ>
					</S01>
				</xsl:if>
				<xsl:if test="count($Signatur)=1">
					<S01>
						<xsl:variable name="Participant" select="$Signatur"/>
						<Elm>
							<SubElm>01</SubElm>
						</Elm>
						<NAD>
							...
							<Elm>
								<SubElm>
									<xsl:if test="count($Participant/m:PersonSurnameName)=0">_</xsl:if>
									<xsl:value-of select="$Participant/m:PersonSurnameName"/>
								</SubElm>
								<SubElm><xsl:value-of select="$Participant/m:PersonGivenName"/></SubElm>
								<SubElm><xsl:value-of select="$Participant/m:PersonTitle"/></SubElm>
								<SubElm/>
								<SubElm/>
								<SubElm>US</SubElm>
							</Elm>
						...
						<SEQ>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="3+count($CCReceiver)+count($FamDoc)"/></SubElm>
							</Elm>
						</SEQ>
					</S01>
				</xsl:if>
				...
				<xsl:for-each select="$ContactInfos">
					<S01>
						...
						<NAD>
							...
							<Elm>
								<SubElm>
									<xsl:if test="count(m:ContactName)>0"><xsl:value-of select="m:ContactName[1]"/></xsl:if>
								</SubElm>
								<SubElm>
									<xsl:if test="count(m:ContactName)>1"><xsl:value-of select="m:ContactName[2]"/></xsl:if>	
								</SubElm>
								<SubElm/>
								<SubElm/>
								<SubElm/>
						...
						<CON>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="m:TelephoneSubscriberIdentifier"/></SubElm>
								<SubElm>TE</SubElm>
							</Elm>
						</CON>
						</xsl:if>
						<xsl:if test="count(m:TelefaxSubscriberIdentifier)=1">
						<CON>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="m:TelefaxSubscriberIdentifier"/></SubElm>
								<SubElm>FX</SubElm>
							</Elm>
						</CON>
						</xsl:if>
						<xsl:if test="count(m:EmailIdentifier)=1">
						<CON>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="m:EmailIdentifier"/></SubElm>
								<SubElm>EM</SubElm>
							</Elm>
						</CON>
						</xsl:if>
						...
						<RFF>
							<Elm>
								<SubElm>BUH</SubElm>
								<SubElm><xsl:value-of select="m:ContactTimeText"/></SubElm>
							</Elm>
						</RFF>
						<SEQ>
							<Elm/>
							<Elm>
								<SubElm><xsl:value-of select="2+position()+count($Signatur)+count($CCReceiver)+count($FamDoc)"/></SubElm>
							</Elm>
						</SEQ>
				...
				<xsl:for-each select="$Relatives">
					<S09>
						...
						<xsl:for-each select="m:CodedTelephoneSubscriberIdentifier">
							<CON>
								<Elm>
									<SubElm>
										<xsl:variable name="Value" select="m:TelephoneCode"/>
										<xsl:choose>
											<xsl:when test="$Value='privat'">HO</xsl:when>
											<xsl:when test="$Value='arbejde'">WO</xsl:when>
											<xsl:when test="$Value='mobil'">MO</xsl:when>
											<xsl:otherwise>
												<FEJL>Kan ikke oversætte kode for telefontype: <xsl:value-of select="$Value"/>
												</FEJL>
											</xsl:otherwise>
										</xsl:choose>
									</SubElm>
				...
				<S14>
					...
					<DTM>
						<Elm>
							<SubElm>CIS</SubElm>
							<SubElm>
								<xsl:variable name="DT" select="$Admission"/>
								<xsl:variable name="DT203" select="concat(substring($DT/m:Date,1,4),substring($DT/m:Date,6,2),substring($DT/m:Date,9,2),substring($DT/m:Time,1,2),substring($DT/m:Time,4,2))"/>
								<xsl:value-of select="$DT203"/>
							</SubElm>
							<SubElm>203</SubElm>
					...
					<xsl:if test="count($EndOfTreatment)=1">
					<DTM>
						<Elm>
							<SubElm>CIE</SubElm>
							<SubElm>
								<xsl:variable name="DT" select="$EndOfTreatment"/>
								<xsl:variable name="DT203" select="concat(substring($DT/m:Date,1,4),substring($DT/m:Date,6,2),substring($DT/m:Date,9,2),substring($DT/m:Time,1,2),substring($DT/m:Time,4,2))"/>
								<xsl:value-of select="$DT203"/>
							</SubElm>
							<SubElm>203</SubElm>
					...
					<xsl:if test="count($PlannedDischarge)=1">
						<DTM>
							<Elm>
								<SubElm>CIF</SubElm>
								<SubElm>
								<xsl:variable name="DT" select="$PlannedDischarge"/>
									<xsl:variable name="DT203" select="concat(substring($DT/m:Date,1,4),substring($DT/m:Date,6,2),substring($DT/m:Date,9,2),substring($DT/m:Time,1,2),substring($DT/m:Time,4,2))"/>
									<xsl:value-of select="$DT203"/>
								</SubElm>
								<SubElm>203</SubElm>
					...
					<xsl:if test="count($PlanConference)=1">
						<DTM>
							<Elm>
								<SubElm>CIP</SubElm>
								<SubElm>
								<xsl:variable name="DT" select="$PlanConference"/>
									<xsl:variable name="DT203" select="concat(substring($DT/m:Date,1,4),substring($DT/m:Date,6,2),substring($DT/m:Date,9,2),substring($DT/m:Time,1,2),substring($DT/m:Time,4,2))"/>
									<xsl:value-of select="$DT203"/>
								</SubElm>
								<SubElm>203</SubElm>
					...
					<xsl:for-each select="$CareDescription">
						<XFTX maxOccurs="9">
							<Elm>
								<SubElm>NC</SubElm>
							</Elm>
							...
							<Elm/>
							<Elm>
								<SubElm><xsl:for-each select="m:CommentsText"><xsl:for-each select="text() | *"><xsl:apply-templates select="."/></xsl:for-each></xsl:for-each>
