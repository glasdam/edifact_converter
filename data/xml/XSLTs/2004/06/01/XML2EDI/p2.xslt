...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/medcom.dk/xml/schemas/2004/06/01/" version="1.0">
	...
	<xsl:template match="m:MicrobiologyReport">
		...
		<xsl:variable name="GrowthWOFindings" select="$LabResults/m:GrowthWithoutFindings"/>
		<xsl:variable name="GrowthWFindings" select="$LabResults/m:GrowthWithFindings"/>
			...
			<BrevIndhold>
				...
				<xsl:for-each select="$MicroOrgs">
					...
					<S18 hidden="true">
						...
						<xsl:if test="count(m:MultiResistancy)>0">
							<FTX>
								<Elm>
									<SubElm>SPC</SubElm>
								</Elm>
								...
								<Elm>
									<SubElm>
										<xsl:value-of select="m:MultiResistancy"/>
									</SubElm>
								</Elm>
							</FTX>
						</xsl:if>
