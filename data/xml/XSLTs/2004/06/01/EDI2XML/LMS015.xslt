	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDPRE' and SubElm[5]='LMS015']]">
		...
		<Prescription>
			...
			<xsl:for-each select="$S03PatOrRel">
				<PatientOrRelative>
					...
					<xsl:for-each select="$S02">
						<PrivateInsurance>
							<xsl:value-of select="count(ICD[Elm[1]/SubElm[1]='DK'])>0"/>
						</PrivateInsurance>
						<LocalAuthorityGrant>
							<xsl:value-of select="count(ICD[Elm[1]/SubElm[1]='MK'])>0"/>
						</LocalAuthorityGrant>
					</xsl:for-each>
				</PatientOrRelative>
			</xsl:for-each>
			...
			<PrescriptionInformation>
				...
				<xsl:for-each select="$S02[RFF[Elm[1]/SubElm[1]='SEP']]">
					<DoseDispenseInformation>
						<Status>seponeres</Status>
					</DoseDispenseInformation>
				</xsl:for-each>
...
<xsl:when test="$RC='SPG'">bevilling_fra_laegemiddelstyrelsen</xsl:when>
					...
					<xsl:for-each select="S07[DTM/Elm[1]/SubElm[1]='90' or Elm[1]/SubElm[1]='91']">
						<DoseDispensing>
