...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/medcom.dk/xml/schemas/2014/10/08/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='PRODAT' and SubElm[5]='A0140Z']]">
		...
		<LaboratoryAnalysisFile>
			<Letter>
				...
				<VersionCode>XA0140Z</VersionCode>
						...
						<xsl:if test="count(FTX[Elm[1]/SubElm[1]='MQ'])>0">
							...
							<xsl:for-each select="FTX[Elm[1]/SubElm[1]='FUN']">
								<LabelTextCode>
										...
										<xsl:choose>
											<xsl:when test="$LTC='0'">uspecificeret</xsl:when>
											<xsl:when test="$LTC='1'">klinisk_kemi_standard</xsl:when>
											<xsl:when test="$LTC='2'">klinisk_kemi_proevetagningstidspunkt</xsl:when>
											<xsl:when test="$LTC='3'">klinisk_kemi_glasgruppenavn</xsl:when>
											<xsl:when test="$LTC='4'">klinisk_kemi_cprnummer</xsl:when>
											<xsl:when test="$LTC='5'">klinisk_kemi_cpr_patientnavn</xsl:when>
											<xsl:when test="$LTC='6'">klinisk_kemi_blodbank</xsl:when>
											<xsl:when test="$LTC='7'">klinisk_kemi_venstrestillet</xsl:when>
											<xsl:when test="$LTC='8'">mikrobiologi_standard</xsl:when>
											<xsl:when test="$LTC='9'">patologi_standard</xsl:when>
											<xsl:otherwise>											
												<xsl:call-template name="ValueListError">
						...
						<xsl:for-each select="FTX[Elm[1]/SubElm[1]='BAR']">
							<BarcodeType>
								...
								<xsl:choose>
									...
									<xsl:when test="$BT='D'">interleaved_2of5_8mm</xsl:when>
									<xsl:when test="$BT='E'">code_128c_8mm</xsl:when>
									<xsl:when test="$BT='F'">code_39_8mmm</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name="ValueListError">
