...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/medcom.dk/xml/schemas/2014/10/08/">
	...
	******************************************************
	...
	<xsl:template name="CONToTelephoneSubscriberIdentifier">
		...
		<xsl:choose>
			<xsl:when test="string-length($CON/Elm[1]/SubElm[1])">
				<TelephoneSubscriber>
					<TelephoneSubscriberIdentifier>
						<xsl:value-of select="$CON/Elm[2]/SubElm[1]"/>
					</TelephoneSubscriberIdentifier>
					<TelephoneSubscriberCode>
						<xsl:choose>
							<xsl:when test="$CON/Elm[1]/SubElm[1] = 'CE'">
								<xsl:text>mobil</xsl:text>
							</xsl:when>
						</xsl:choose>
						<xsl:choose>
							<xsl:when test="$CON/Elm[1]/SubElm[1] = 'HO'">
								<xsl:text>fastnet</xsl:text>
							</xsl:when>
						</xsl:choose>
						<xsl:choose>
							<xsl:when test="$CON/Elm[1]/SubElm[1] = 'WO'">
								<xsl:text>arbejde</xsl:text>
							</xsl:when>
						</xsl:choose>
					</TelephoneSubscriberCode>
				</TelephoneSubscriber>
	...
	<xsl:template name="ValueToObjectExtensionCode">
		...
		<ObjectExtensionCode>
			<xsl:choose>
				...
				<xsl:when test="$OEC='FNX'">fnx</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="ValueListError">
