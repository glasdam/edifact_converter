...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/medcom.dk/xml/schemas/2014/10/08/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDRPT' and SubElm[5]='R0130F']]">
		...
		<DiabeticFootStatus>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XR0130F</VersionCode>
			...
			<xsl:for-each select="$S01Sender">
				<Sender>
					...
					<xsl:for-each select="NAD">
						...
						<xsl:call-template name="NADToIdentifierCodeOrLocalFoot"/>
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:call-template name="Error">
										...
										<xsl:with-param name="Text">Kan ikke overætte fra SERVICETYPE:<xsl:value-of select="$RServC"/> og STATUS 2: <xsl:value-of select="$RStatC"/> til ResultServiceCode
										</xsl:with-param>
									</xsl:call-template>
								</xsl:otherwise>
							</xsl:choose>
	...
	<xsl:template name="NADToIdentifierCodeOrLocalFoot">
