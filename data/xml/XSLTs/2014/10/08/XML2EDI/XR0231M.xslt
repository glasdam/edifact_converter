			 ...
			 <xsl:otherwise>
				...
				<S02>
					...
					<STS>
						...
						<Elm>
							<SubElm>
								<xsl:variable name="RS" select="$GeneralResultInformation/m:ReportStatusCode"/>
								<xsl:choose>
									<xsl:when test="$RS='komplet_svar'">K</xsl:when>
									<xsl:when test="$RS='del_svar'">D</xsl:when>
									<xsl:when test="$RS='modtaget'">M</xsl:when>
									<xsl:otherwise>
										<FEJL>Ukendt ReportStatusCode: <xsl:value-of select="$RS"/>
										</FEJL>
									</xsl:otherwise>
								</xsl:choose>
							</SubElm>
