...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:m="http://rep.oio.dk/medcom.dk/xml/schemas/2012/03/28/" version="1.0">
			...
			<UNB>
				...
				<Elm>
					<SubElm>
						...
						<xsl:variable name="Datoyymmdd"
							select="concat(substring($Dato,3,2),substring($Dato,6,2),substring($Dato,9,2))"/>
						<xsl:value-of select="$Datoyymmdd"/>
					</SubElm>
					<SubElm>
						<xsl:variable name="Tid" select="$Envelope/m:Sent/m:Time"/>
						<xsl:variable name="Tidhhmm"
							select="concat(substring($Tid,1,2),substring($Tid,4,2))"/>
				...
				<xsl:variable name="IsCtrl"
					select="substring($FirstMsg/m:Letter/m:VersionCode,1,2)='XC' "/>
					...
					<Elm>
						<SubElm>
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<FEJL>Kan ikke oversætte AcknowledgementCode: <xsl:value-of
											select="$C"/>
									</FEJL>
								</xsl:otherwise>
							</xsl:choose>
				...
				<xsl:if test="$IsCtrl">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<FEJL>AcknowledgementCode værdien er ikke tilladt i CONTROL:
									<xsl:value-of select="$C"/></FEJL>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
			</UNB>
