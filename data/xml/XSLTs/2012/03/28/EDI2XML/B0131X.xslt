...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns="http://rep.oio.dk/medcom.dk/xml/schemas/2012/03/28/">
	<xsl:output method="xml" version="1.0" encoding="UTF-8" indent="yes"/>
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDBIN' and SubElm[5]='B0131X']]">
		...
		<BinaryLetter>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XB0131X</VersionCode>
			...
			<xsl:for-each select="$S11Binary">
				<xsl:for-each select="UNO">
					<BinaryObject>
						<ObjectIdentifier>
							<xsl:variable name="mcuuid" select="Elm[2]/SubElm[2]"/>
							<xsl:value-of select="substring($mcuuid, 1, 8)"/>-<xsl:value-of select="substring($mcuuid, 9, 4)"/>-<xsl:value-of select="substring($mcuuid, 13, 4)"/>-<xsl:value-of select="substring($mcuuid, 17, 4)"/>-<xsl:value-of select="substring($mcuuid, 21, 12)"/>
						</ObjectIdentifier>
