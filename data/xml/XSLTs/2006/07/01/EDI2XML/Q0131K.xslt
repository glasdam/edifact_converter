...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDREQ' and SubElm[5]='Q0131K']]">
		...
		<xsl:variable name="S01Sender"
			select="BrevIndhold/S01/NAD[Elm[1]/SubElm[1]='PO'] | BrevIndhold/S01/NAD[Elm[1]/SubElm[1]='ONL']"/>
	    ...
	    |	BrevIndhold/S01[not(NAD/Elm[1]/SubElm[1]='PO' or NAD/Elm[1]/SubElm[1]='SLA' or NAD/Elm[1]/SubElm[1]='CCR' or NAD/Elm[1]/SubElm[1]='BV' or NAD/Elm[1]/SubElm[1]='PAY' or NAD/Elm[1]/SubElm[1]='ORL' or NAD/Elm[1]/SubElm[1]='ONL' or NAD/Elm[1]/SubElm[1]='ORN')]"/>
		...
		<LaboratoryRequest>
				...
				<Sender>
					...
					<xsl:for-each select=".">
						<ReplyTo>
							...
							<xsl:choose>
								<xsl:when test="$RTO='ONL'">false</xsl:when>
								<xsl:when test="$RTO='PO'">true</xsl:when>
								<xsl:otherwise>
			...
			<xsl:for-each select="BrevIndhold/S01/NAD[Elm[1]/SubElm[1]='PAY']">
				<Payer>
					<PayersTypeCode>
					...
					<xsl:for-each select=".">
						...
						<xsl:if
							test="$S02LetterInfo/FCA/Elm[1]/SubElm[1]='NSP' and Elm[2]/SubElm[1]!='' ">
							<xsl:call-template name="Error">
								<xsl:with-param name="Node" select="Elm[2]/SubElm[1]"/>
								<xsl:with-param name="Text">Der kan ikke angives BETID når BETKOD er
										<xsl:value-of select="$S02LetterInfo/FCA/Elm[1]/SubElm[1]"/>
								</xsl:with-param>
							</xsl:call-template>
						</xsl:if>
						<xsl:if
							...
							<xsl:call-template name="Error">
								<xsl:with-param name="Node" select="Elm[2]/SubElm[1]"/>
								<xsl:with-param name="Text">Der skal angives BETID når BETKOD er
										<xsl:value-of select="$S02LetterInfo/FCA/Elm[1]/SubElm[1]"/>
								</xsl:with-param>
							</xsl:call-template>
						</xsl:if>
									...
									<xsl:otherwise>
										<xsl:call-template name="Error">
											...
											<xsl:with-param name="Text">Kan ikke oversaette til
												IdentifierCodeType: <xsl:value-of select="$IC"/>
											</xsl:with-param>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
			...
			<RequisitionInformation>
				...
				<xsl:for-each select="$S15Sample[1]/SPC[Elm[1]/SubElm[1]='SCI']">
					<SamplingLocationCode>
						...
						<xsl:choose>
							...
							<xsl:when test="$SLC='EXT'">laboratoriet_eksternt</xsl:when>
							<xsl:otherwise>
								<xsl:call-template name="ValueListError">
				...
				<xsl:for-each select="$S02LetterInfo/FTX[Elm[1]/SubElm[1]='EXT']">
					<xsl:if test="$S15Sample[1]/SPC[Elm[1]/SubElm[1]='SCI']/Elm[2]/SubElm[1]!='EXT'">
						<xsl:call-template name="Error">
							<xsl:with-param name="Node" select="."/>
							<xsl:with-param name="Text">FTX-EXT må kun angives hvis PRKODE = EXT.
							</xsl:with-param>
						</xsl:call-template>
					</xsl:if>
					<SamplingLocation>
				...
				<xsl:for-each select="$S15Sample[1]/SPC[Elm[1]/SubElm[1]='SCI']">
					<SamplingDateTimeType>
						...
						<xsl:choose>
							...
							<xsl:when test="$SLC='EXT'">oensket</xsl:when>
							<xsl:otherwise>
								<xsl:call-template name="ValueListError">
									<xsl:with-param name="ValueNode" select="$SLC"/>
									<xsl:with-param name="Text">Ukendt PRKODE</xsl:with-param>
				...
				<xsl:for-each select="$S15Sample/DTM">
					<SamplingDateTime>
						<xsl:call-template name="DTM203ToDateTime"/>
					</SamplingDateTime>
				</xsl:for-each>
				<xsl:for-each select="$S15Sample">
					<Sample>
						...
						<xsl:variable name="RefFTXs"
							select="FTX[Elm[1]/SubElm[1]='BIN' or Elm[1]/SubElm[1]='SUP' or Elm[1]/SubElm[1]='URL']"/>
				...
				<xsl:for-each
					select="$S02LetterInfo/FTX[Elm[1]/SubElm[1]!='KOP' and Elm[1]/SubElm[1]!='EXT']">
					<Comments>
						<xsl:call-template name="FTXSegmentsToBreakableText"/>
					</Comments>
				</xsl:for-each>
					...
					<RequestedAnalysis>
						...
						<xsl:for-each select="INV">
							<Analysis>
								...
								<xsl:for-each select="Elm[2]/SubElm[4]">
									<AnalysisShortName>
										<xsl:value-of select="."/>
									</AnalysisShortName>
								</xsl:for-each>
						...
						<xsl:for-each select="../FTX[Elm[1]/SubElm[1]='SRC']">
							<SampleLocation>
								<xsl:value-of select="Elm[4]/SubElm"/>
							</SampleLocation>
						</xsl:for-each>
						<xsl:for-each select="../FTX[Elm[1]/SubElm[1]='SMA']">
							<SampleMaterial>
								<xsl:value-of select="Elm[4]/SubElm"/>
							</SampleMaterial>
						</xsl:for-each>
						</xsl:for-each>
						<xsl:for-each select="RFF[Elm[1]/SubElm[1]='SID'] ">
							<TestTubeIdentifier>
								<xsl:value-of select="Elm[1]/SubElm[2]"/>
							</TestTubeIdentifier>
						  <xsl:if test="Elm[1]/SubElm[4]">
