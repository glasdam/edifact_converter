...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2006/07/01/">
	<xsl:output method="xml" version="1.0" encoding="UTF-8" indent="yes"/>
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='PRODAT' and SubElm[5]='A0135Z']]">
		...
		<LaboratoryAnalysisFile>
			<Letter>
				...
				<VersionCode>XA0135Z</VersionCode>
							...
							<xsl:for-each select="FTX[Elm[1]/SubElm[1]='FUN']">
								<LabelTextCode>
										...
										<xsl:choose>
											...
											<xsl:when test="$LTC='10'">ydernummer</xsl:when>
											<xsl:when test="$LTC='11'">proevetagningstidspunkt</xsl:when>
											<xsl:when test="$LTC='12'">glasgruppenummer</xsl:when>
											<xsl:when test="$LTC='13'">cprnummer</xsl:when>
											<xsl:otherwise>											
												<xsl:call-template name="ValueListError">
