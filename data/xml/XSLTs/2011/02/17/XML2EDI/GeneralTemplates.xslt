...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/medcom.dk/xml/schemas/2011/02/17/" version="1.0">
  ...
  <xsl:template name="MedicalSpecialityCodeToAFSSPEC">
    ...
    <xsl:if test="count($MSC)=1">
      <xsl:choose>
        ...
        <xsl:when test=" $MSC='dermato_venerologi_praksis'">2504</xsl:when>
  ...
  <xsl:template name="SpecialityCodeToAFSSPEC">
    ...
    <xsl:if test="count($SC)=1">
      <xsl:choose>
        <xsl:when test=" $SC='anaestesi'">1</xsl:when>
        <xsl:when test=" $SC='diagnostisk_radiologi_koebenhavn'">3</xsl:when>
        <xsl:when test=" $SC='dermatologi_venerologi'">4</xsl:when>
        <xsl:when test=" $SC='radiologi'">5</xsl:when>
        <xsl:when test=" $SC='reumatologi'">6</xsl:when>
        <xsl:when test=" $SC='gynaekologi_obstetrik'">7</xsl:when>
        <xsl:when test=" $SC='intern_medicin'">8</xsl:when>
        <xsl:when test=" $SC='kirurgi'">9</xsl:when>
