...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		<FEJL>
			<xsl:if test="$Node/@linie!='' ">
				<xsl:attribute name="linie">
					<xsl:value-of select="$Node/@linie"/>
				</xsl:attribute>
			</xsl:if>
			<xsl:if test="$Node/@position!='' ">
				<xsl:attribute name="position">
					<xsl:value-of select="$Node/@position"/>
				</xsl:attribute>
			</xsl:if>
	...
	******************************************************************************************************
	...
	<xsl:template match="Brev">
		<xsl:call-template name="Error">
			...
			<xsl:with-param name="Text">Kan ikke konvertere breve af typen: [<xsl:value-of
					select="UNH/Elm[2]/SubElm[1]"/> , <xsl:value-of select="UNH/Elm[2]/SubElm[5]"
				/>]</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	<!-- 
	...
	*********************************************************************
	...
	<xsl:template name="DTM203ToDateTime">
		...
		<xsl:if test="string-length($DT203)!=12">
			<xsl:call-template name="Error">
				<xsl:with-param name="Text">Forkert antal tegn i Datotid:<xsl:value-of
						select="$DT203"/> - der skal være 12 når kvalifikatoren er
					203</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		<Date>
	...
	*********************************************************************
	...
	<xsl:template name="DTM204ToDateTimeWithSec">
		...
		<xsl:if test="string-length($DT204)!=14">
			<xsl:call-template name="Error">
				<xsl:with-param name="Text">Forkert antal tegn i Datotid:<xsl:value-of
						select="$DT204"/> - der skal være 14 når kvalifikatoren er
					204</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		<Date>
			<xsl:value-of
				select="concat(substring($DT204,1,4),'-',substring($DT204,5,2),'-',substring($DT204,7,2))"
			/>
		</Date>
		<TimeWithSec>
			<xsl:value-of
				select="concat(substring($DT204,9,2),':',substring($DT204,11,2),':',substring($DT204,13,2))"
			/>
		</TimeWithSec>
	</xsl:template>
	...
	*********************************************************************
	...
	<xsl:template name="DTM102ToDateType">
		...
		<xsl:if test="string-length($DT102)!=8">
			<xsl:call-template name="Error">
				<xsl:with-param name="Text">Forkert antal tegn i Datotid:<xsl:value-of
						select="$DT102"/> - der skal være 8 når kvalifikatoren er
					102</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		<xsl:value-of
			select="concat(substring($DT102,1,4),'-',substring($DT102,5,2),'-',substring($DT102,7,2))"
		/>
	</xsl:template>
	<!--
		*********************************************************************
		* Template der oversætter DTM segmenter af typen 103 *
		*********************************************************************
	-->
	<xsl:template name="DTM103ToYearsWeeksDays">
		<xsl:param name="DTM" select="."/>
		<xsl:if test="$DTM/Elm[1]/SubElm[3]!='103' ">
			<xsl:call-template name="Error">
				<xsl:with-param name="Node" select="$DTM/Elm[1]/SubElm[3]"/>
				<xsl:with-param name="Text">Kun datotider af typen 103 er tilladt</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		<xsl:variable name="DT103" select="$DTM/Elm[1]/SubElm[2]"/>
		<xsl:if test="string-length($DT103)!=5">
			<xsl:call-template name="Error">
				<xsl:with-param name="Text">Forkert antal tegn i Datotid:<xsl:value-of
						select="$DT103"/> - der skal være 5 når kvalifikatoren er
					102</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="substring($DT103,1,2) &gt;'00' and substring($DT103,1,2) &lt;'99'">
			<Years>
				<xsl:value-of select="substring($DT103,1,2)"/>
			</Years>
		</xsl:if>
		<xsl:if test="substring($DT103,3,2) &gt;'00' and substring($DT103,3,2) &lt;'54'">
			<Weeks>
				<xsl:value-of select="substring($DT103,3,2)"/>
			</Weeks>
		</xsl:if>
		<xsl:if test="substring($DT103,5,1) &gt;'0' and substring($DT103,5,1) &lt;'8'">
		<Days>
			<xsl:value-of select="substring($DT103,5,1)"/>
		</Days>
		</xsl:if>
	</xsl:template>
	<!-- 
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:call-template name="Error">
					...
					<xsl:with-param name="Text">Ukendt datoformat:<xsl:value-of
							select="$DTM/Elm[1]/SubElm[3]"/>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	...
	*********************************************************
		...
		<xsl:if test="$AFSSPEC!='' ">
			<MedicalSpecialityCode>
				<xsl:choose>
					...
					<xsl:when test=" $AFSSPEC='80' ">paediatri_sygehus</xsl:when>
					...
					<xsl:otherwise>
						<xsl:call-template name="ValueListError">
							...
							<xsl:with-param name="Text">Kan ikke oversætte AFSSPEC til
								MedicalSpecialityCode</xsl:with-param>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
			</MedicalSpecialityCode>
	...
	<xsl:template name="AFSSPECToSpecialityCode">
		<xsl:param name="SC" select="."/>
		<xsl:if test="count($SC)=1">
			<xsl:choose>
				<xsl:when test=" $SC=1">anaestesi</xsl:when>
				<xsl:when test=" $SC=3">diagnostisk_radiologi_koebenhavn</xsl:when>
				<xsl:when test=" $SC=4">dermatologi_venerologi</xsl:when>
				<xsl:when test=" $SC=5">radiologi</xsl:when>
				<xsl:when test=" $SC=6">reumatologi</xsl:when>
				<xsl:when test=" $SC=7">gynaekologi_obstetrik</xsl:when>
				<xsl:when test=" $SC=8">intern_medicin</xsl:when>
				<xsl:when test=" $SC=9">kirurgi</xsl:when>
				<xsl:when test=" $SC=18">neuromedicin</xsl:when>
				<xsl:when test=" $SC=19">oejenlaege</xsl:when>
				<xsl:when test=" $SC=20">ortopaedisk_kirurgi</xsl:when>
				<xsl:when test=" $SC=21">oere_naese_halslaege</xsl:when>
				<xsl:when test=" $SC=22">patologi</xsl:when>
				<xsl:when test=" $SC=23">plastikkirurgi</xsl:when>
				<xsl:when test=" $SC=24">psykiatri</xsl:when>
				<xsl:when test=" $SC=25">paediatri</xsl:when>
				<xsl:when test=" $SC=26">boernepsykiatri</xsl:when>
				<xsl:when test=" $SC=28">tropemedicin</xsl:when>
				<xsl:when test=" $SC=50">tandlaege</xsl:when>
				<xsl:when test=" $SC=51">fysioterapi</xsl:when>
				<xsl:when test=" $SC=53">kiropraktor</xsl:when>
				<xsl:when test=" $SC=54">fodterapi_sukkersyge</xsl:when>
				<xsl:when test=" $SC=55">fodterapi_unquis_incarnatus</xsl:when>
				<xsl:when test=" $SC=57">ridefysioterapi</xsl:when>
				<xsl:when test=" $SC=59">fodterapi_foelger_radioaktiv_bestraaling</xsl:when>
				<xsl:when test=" $SC=62">vederlagsfri_fysioterapi</xsl:when>
				<xsl:when test=" $SC=63">psykolog</xsl:when>
				<xsl:when test=" $SC=64">kiropraktik_ordning_64</xsl:when>
				<xsl:when test=" $SC=65">vederlagsfri_ridefysioterapi</xsl:when>
				<xsl:when test=" $SC='80'">alment_praktiserende_laege</xsl:when>
				<xsl:otherwise>
					<FEJL>Ukendt afsenderspeciale: <xsl:value-of select="$SC"/>
					</FEJL>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	...
	<xsl:template name="RELToRelationCode">
		...
		<RelationCode>
			...
			<xsl:choose>
				...
				<xsl:when test="$RC='PA'">aegtefaelle</xsl:when>
				<xsl:when test="$RC='SD'">barn</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="ValueListError">
						<xsl:with-param name="ValueNode" select="$RC"/>
						<xsl:with-param name="Text">Kan ikke oversætte kode for
							pårørende</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
		</RelationCode>
	...
	<xsl:template name="NADToIdentifierCode">
		...
		<IdentifierCode>
			...
			<xsl:choose>
				...
				<xsl:when test="$IC='NCD' ">lokalt_nummer</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="Error">
						<xsl:with-param name="Node" select="$IC"/>
						<xsl:with-param name="Text">Kan ikke oversaette til IdentifierCodeType:
	...
	***********************************
	...
	<xsl:template name="SpecialPersonName">
		<xsl:param name="NAD" select="."/>
		<xsl:if test="$NAD/Elm[4]/SubElm[1]!=''">
			<PersonName>
				<xsl:value-of select="$NAD/Elm[4]/SubElm[1]"/>
	...
	<xsl:template name="SpecialOrganisationName">
		<xsl:param name="NAD" select="."/>
		<xsl:if test="$NAD/Elm[4]/SubElm[2]!=''">
			<OrganisationName>
				<xsl:value-of select="$NAD/Elm[4]/SubElm[2]"/>
	...
	******************************************************
	...
	<xsl:template name="PNAToPersonSurnameNameNM">
		<!-- Hvis efternavn ikke er mandatory-->
		<xsl:param name="PNA" select="."/>
	...
	******************************************************
	...
	<xsl:template name="RFFToBirthStatus1">
		<xsl:param name="RFF" select="."/>
		<xsl:if test="$RFF/Elm[1]/SubElm[1]='K01' ">
			<BirthStatus1>
				<xsl:value-of select="$RFF/Elm[1]/SubElm[2]"/>
			</BirthStatus1>
		</xsl:if>
	</xsl:template>

	<xsl:template name="RFFToBirthStatus2">
		<xsl:param name="RFF" select="."/>
		<xsl:if test="$RFF/Elm[1]/SubElm[1]='K02' ">
			<BirthStatus2>
				<xsl:value-of select="$RFF/Elm[1]/SubElm[2]"/>
			</BirthStatus2>
		</xsl:if>
	...
	******************************************************
	...
	<!-- 
		...
		******************************************************
	-->
	<xsl:template name="CONToTelephoneSubscriber">
		<xsl:param name="CON" select="."/>
			<xsl:variable name="TS" select="$CON/Elm[1]/SubElm[1]"/>
			<xsl:choose>
				<xsl:when test="$TS='HO'">
					<xsl:element name="PrivateSubscriberIdentifier">
						<xsl:value-of select="$CON/Elm[2]/SubElm[1]"/>
					</xsl:element>
				</xsl:when>
				<xsl:when test="$TS='WO'">
					<xsl:element name="WorkSubscriberIdentifier">
						<xsl:value-of select="$CON/Elm[2]/SubElm[1]"/>
					</xsl:element>
				</xsl:when>
				<xsl:when test="$TS='MO'">
					<xsl:element name="MobileSubscriberIdentifier">
						<xsl:value-of select="$CON/Elm[2]/SubElm[1]"/>
					</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="ValueListError">
						<xsl:with-param name="ValueNode" select="$TS"/>
						<xsl:with-param name="Text">Ukendt TelephoneSubscriberCode</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
	...
	******************************************************
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:call-template name="Error">
						...
						<xsl:with-param name="Text">Kan ikke oversaette til PatientSex:
	...
	<xsl:template name="PDIToGenderChild">
		<xsl:param name="PDI" select="."/>
		<Gender>
			<xsl:variable name="PS" select="$PDI/Elm[1]/SubElm[1]"/>
			<xsl:choose>
				<xsl:when test="$PS='1' ">dreng</xsl:when>
				<xsl:when test="$PS='2' ">pige</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="Error">
						<xsl:with-param name="Node" select="$PS"/>
						<xsl:with-param name="Text">Kan ikke oversaette til GenderChild:
								<xsl:value-of select="$PS"/>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
	...
	<xsl:template name="PDIToMaritalStatus">
		<xsl:param name="PDI" select="."/>
		<MaritalStatus>
			<xsl:variable name="PS" select="$PDI/Elm[2]/SubElm[1]"/>
			<xsl:choose>
				<xsl:when test="$PS='MA' ">gift</xsl:when>
				<xsl:when test="$PS='SI' ">single</xsl:when>
				<xsl:when test="$PS='SP' ">separeret</xsl:when>
				<xsl:when test="$PS='WI' ">enke</xsl:when>
				<xsl:when test="$PS='DI' ">skilt</xsl:when>
				<xsl:when test="$PS='LI' ">samboende</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="Error">
						<xsl:with-param name="Node" select="$PS"/>
						<xsl:with-param name="Text">Kan ikke oversaette til MaritalStatus:
								<xsl:value-of select="$PS"/>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
	...
	******************************************************
	...
	<xsl:template name="CINToDiagnoseDescriptionCode">
		<xsl:param name="CIN" select="."/>
		<DiagnoseDescriptionCode>
			<xsl:variable name="CI" select="$CIN/Elm[1]/SubElm[1]"/>
			<xsl:choose>
				<xsl:when test="$CI='DI' ">uspec_diagnose</xsl:when>
				<xsl:when test="$CI='H' ">henv_diagnose</xsl:when>
				<xsl:when test="$CI='B' ">bidiagnose</xsl:when>
				<xsl:when test="$CI='DM' ">tillaegsdiagnose</xsl:when>
				<xsl:when test="$CI='A' ">aktionsdiagnose</xsl:when>
				<xsl:when test="$CI='G' ">grundmorbus</xsl:when>
				<xsl:when test="$CI='M' ">midlertidig_diagnose</xsl:when>
				<xsl:when test="$CI='S' ">forloebsdiagnose</xsl:when>
				<xsl:when test="$CI='TR' ">operation</xsl:when>
				<xsl:when test="$CI='IN'">roentgenundersoegelse</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="Error">
						<xsl:with-param name="Node" select="$CI"/>
						<xsl:with-param name="Text">Kan ikke oversaette til DiagnoseDescriptionCode:
	...
	<xsl:template name="CINToDiagnoseCode">
		<xsl:param name="CIN" select="."/>
		<DiagnoseCode>
			<xsl:value-of select="$CIN/Elm[2]/SubElm[1]"/>
		</DiagnoseCode>
	</xsl:template>
	<xsl:template name="CINToDiagnoseTypeCode">
		<xsl:param name="CIN" select="."/>
		<DiagnoseTypeCode>
			<xsl:variable name="CI" select="$CIN/Elm[2]/SubElm[2]"/>
			<xsl:choose>
				<xsl:when test="$CI='SKS' ">SKSdiagnosekode</xsl:when>
				<xsl:when test="$CI='USP' ">uspecificeretkode</xsl:when>
				<xsl:when test="$CI='ICP' ">ICPCkod</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="Error">
						<xsl:with-param name="Node" select="$CI"/>
						<xsl:with-param name="Text">Kan ikke oversaette til DiagnoseTypeCode:
								<xsl:value-of select="$CI"/>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
	...
	<xsl:template name="CINToDiagnoseText">
		<xsl:param name="CIN" select="."/>
		<DiagnoseText>
			<xsl:value-of select="$CIN/Elm[2]/SubElm[4]"/>
		</DiagnoseText>
			...
			<xsl:choose>
				...
				<xsl:when test="$RefType='URL' ">
					<URL>
						<xsl:call-template name="FTXSubElmsToText">
							<xsl:with-param name="FTXSubElms"
								select="$FTX/Elm[4]/SubElm[position()>1]"/>
						</xsl:call-template>
					</URL>
				</xsl:when>
				...
				<xsl:otherwise>
					<xsl:call-template name="Error">
						...
						<xsl:with-param name="Text">Kan ikke oversaette til Referencetype:
	...
	*******************************************************************
	...
	<!-- 
		******************************************************
		* Templates til at oversætte LAN segmenter *
		******************************************************
	-->
	<xsl:template name="LANToInterpretationIsNeededCode">
		<xsl:param name="LAN" select="."/>
		<xsl:variable name="IINC" select="$LAN/Elm[1]/SubElm[1]"/>
		<xsl:if test="$LAN/Elm[1]/SubElm[1]!='' and $IINC!='TJ'">
			<InterpretationIsNeededCode>
				<xsl:choose>
					<xsl:when test="$IINC='TJ' ">tolkebehov_ja</xsl:when>
					<xsl:when test="$IINC='TN' ">tolkebehov_nej</xsl:when>
					<xsl:when test="$IINC='NO' ">tolkebehov_uafklaret</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="Error">
							<xsl:with-param name="Node" select="$IINC"/>
							<xsl:with-param name="Text">Kan ikke oversaette til
								InterpretationIsNeededCode: <xsl:value-of select="$IINC"/>
							</xsl:with-param>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
	...
	<xsl:template name="LANToISOLanguageCode">
		<xsl:param name="LAN" select="."/>
		<xsl:if test="$LAN/Elm[2]/SubElm[1]!=''">
			<ISOLanguageCode>
				<xsl:value-of select="$LAN/Elm[2]/SubElm[1]"/>
			</ISOLanguageCode>
		</xsl:if>
	</xsl:template>
	<xsl:template name="LANToISOLanguageCodeDescription">
		<xsl:param name="LAN" select="."/>
		<xsl:if test="$LAN/Elm[2]/SubElm[2]!=''">
			<ISOLanguageCodeDescription>
				<xsl:value-of select="$LAN/Elm[2]/SubElm[2]"/>
			</ISOLanguageCodeDescription>
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>
