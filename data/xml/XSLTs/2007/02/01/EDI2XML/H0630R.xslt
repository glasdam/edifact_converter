...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDREF' and SubElm[5]='H0630R']]">
		...
		<xsl:variable name="RefDiag" select="$S02LetterInfo/CIN[Elm[1]/SubElm[1]='HI']"/>
		<xsl:variable name="RefAddDiags" select="$S02LetterInfo/CIN[Elm[1]/SubElm[1]!='HI']"/>
				...
				<Refer>
					...
					<DiagnoseTypeCode>
						...
						<xsl:choose>
							...
							<xsl:when test="$DTC='HET' ">speciallaegehenvisning</xsl:when> <!-- kan også oversættes til fysioterapeuthenvisning/psykologhenvisning -->
							<xsl:otherwise>
								<xsl:call-template name="Error">
