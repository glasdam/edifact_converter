...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDRPT' and SubElm[5]='R0430P']]">
		...
		<HistopathologyReport>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XR0430P</VersionCode>
				...
				<TextualFormat>
					...
					<xsl:for-each select="$S18Hematology">
						<Hematology>
							...
							<CellTypes>
								<xsl:value-of select="FTX[1]/Elm[4]/SubElm[1]"/>
							</CellTypes>
							<xsl:for-each select="FTX[1]/Elm[4]/SubElm[position()>1]">
								<CellTypesPercentage>
									<xsl:value-of select="."/>
								</CellTypesPercentage>
							</xsl:for-each>
