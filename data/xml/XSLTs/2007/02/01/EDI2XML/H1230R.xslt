...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDREF' and SubElm[5]='H1230R']]">
		...
		<TeddyReferral>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XH1230R</VersionCode>
					<StatisticalCode>REF12</StatisticalCode>
					<xsl:for-each select="$Dtm">
						<Authorisation>
							<xsl:call-template name="DTM203ToDateTime"/>
						</Authorisation>
					</xsl:for-each>
					<TypeCode>XREF12</TypeCode>
					<StatusCode>
	...
	<xsl:template name="DiacodeToPhysiotherapyDiagnosisTextTemplate2">
		...
		<xsl:choose>
			<xsl:when test="not($DT)">
				<xsl:choose>
					...
					<xsl:when test=" $DC='1K' ">Andre medfødte eller arvelige sygdomme med motoriske handicaps som fx adrenoleukodystrofi, Spielmeyers-Vogts syndrom, familiær amyloidose</xsl:when>
					...
					<xsl:when test=" $DC='1O' ">Arvelige bindevævssygdomme som fx osteogenesis imperfecta, Marfans syndrom, Ehlers Danlos syndrom, Morquios syndrom</xsl:when>
					...
					<xsl:when test=" $DC='2A' ">Følger efter hjerte-, rygmarvs- og hjernehindebetændelse (encephalitis, myelitis, meningitis)</xsl:when>
					<xsl:when test=" $DC='2B' ">Neurologiske kompl. til AIDS</xsl:when>
					<xsl:when test=" $DC='2C' ">Følger efter poliomyelitis</xsl:when>
					<xsl:when test=" $DC='2D' ">Følger efter hjerneblødning (apoplexia cerebri incl. subarachnoidalblødning)</xsl:when>
					...
					<xsl:when test=" $DC='4D' ">Kronisk polyarthritis fx som følge af LED, morbus Sjøgren, morbus Reiter</xsl:when>
