...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDREF' and SubElm[5]='H1030R']]">
		...
		<xsl:variable name="S01AnswerCCReceiver" select="BrevIndhold/S01[NAD/Elm[1]/SubElm[1]='PRH']"/>  <!-- ubrugt -->
		...
		<xsl:variable name="S10Relatives" select="BrevIndhold/S10"/>  <!-- ubrugt -->
		...
		<xsl:variable name="Priority" select="$S18/PTY"/>            <!-- ubrugt -->
		<xsl:variable name="HospitalVisitations" select="$S18/FTX"/>  <!-- ubrugt -->
		<xsl:variable name="RefStatus" select="$S18/PAS"/>		
		<xsl:variable name="S19" select="BrevIndhold/S19"/>
		<xsl:variable name="Transport" select="$S19/TDT"/>             <!-- ubrugt -->
				...
				<Refer>
					...
					<DiagnoseTypeCode>
						...
						<xsl:choose>
							...
							<xsl:when test="$DTC='HET' ">psykologhenvisning</xsl:when> <!-- kan også oversættes til speciallægehenvisning/fysioterapeuthenvisning -->
				...
				<ReferralAdditional>
					<DiagnoseDescriptionCode>
						...
						<xsl:choose>
							...
							<xsl:when test="$Desc='HI' ">psykologhenvisning</xsl:when>	<!-- kan også oversættes til speciallægehenvisning/fysioterapeuthenvisning -->
							<xsl:otherwise>
								<xsl:call-template name="Error">
					...
					<DiagnoseTypeCode>
						...
						<xsl:choose>
							...
							<xsl:when test="$DTC='USP' ">uspecificeretkode</xsl:when>
							<xsl:when test="$DTC='ICP' ">ICPCkode</xsl:when>
							<xsl:when test="$DTC='HET' ">psykologhenvisning</xsl:when> <!-- HET kan også oversættes til speciallægehenvisning og fysioterapeuthenvisning -->
							<xsl:when test="$DTC='SFU' ">Psykologhenvisningsdiagnose</xsl:when> <!-- Kan også oversættes til fysioterapeuthenvisningdiagnose -->
							<xsl:otherwise> 
								<xsl:call-template name="Error">
									<xsl:with-param name="Node" select="$DTC"/>
					...
					<DiagnoseText>
						...
						<xsl:choose>
							...
							<xsl:when test="$DTC='SFU' ">
								<xsl:call-template name="DiacodeToPsychologistDiagnosisTextTemplate">
		...
		<xsl:template name="DiacodeToPsychologistDiagnosisTextTemplate">
			<xsl:param name="DC" select="Elm[2]/SubElm[1]"/>
			<xsl:param name="DT" select="Elm[2]/SubElm[4]"/>
			<xsl:choose>
				<xsl:when test="not($DT)">
					<xsl:choose>
						<xsl:when test=" $DC='1' ">Røveri-, volds – eller voldtægtsoffer</xsl:when>
						<xsl:when test=" $DC='2' ">Trafik eller ulykkesoffer</xsl:when>
						<xsl:when test=" $DC='3' ">Pårørende til alvorligt psykisk syge personer</xsl:when>
						<xsl:when test=" $DC='4' ">Personer ramt af alvorligt invaliderende sygdom</xsl:when>
						<xsl:when test=" $DC='5' ">Pårørende til personer ramt af invaliderende sygdom</xsl:when>
						<xsl:when test=" $DC='6' ">Pårørende ved dødsfald</xsl:when>
						<xsl:when test=" $DC='7' ">Personer der har forsøgt selvmord</xsl:when>
						<xsl:when test=" $DC='8' ">Kvinder, der får foretaget provokeret abort efter 12. graviditetsuge.</xsl:when>
						<xsl:when test=" $DC='9' ">Personer der inden det fyldte 18. år har været ofre for incest.</xsl:when>
						<xsl:when test=" $DC='10' ">Personer m/let->moderat depression (over 18 år), efter psykom. test.</xsl:when>
						<xsl:when test=" $DC='11' ">Personer m/let->moderat angst, herunder let->moderat ocd (18-38 år)</xsl:when>
						<xsl:when test=" $DC='99' ">Fravigelse af 6 mdrs. fristen.</xsl:when>
				  		<xsl:otherwise>
							<xsl:call-template name="ValueListError">
								<xsl:with-param name="ValueNode" select="$DC"/>
								<xsl:with-param name="Text">Kan ikke oversætte CIN til PsychologistDiagnosisCodeText</xsl:with-param>
							</xsl:call-template>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
