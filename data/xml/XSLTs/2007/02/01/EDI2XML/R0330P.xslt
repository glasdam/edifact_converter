...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/">
	...
	<xsl:template match="Brev[UNH/Elm[2][SubElm[1]='MEDRPT' and SubElm[5]='R0330P']]">
		...
		<CervixcytologyReport>
			<xsl:for-each select="$S02LetterInfo">
				<Letter>
					...
					<VersionCode>XR0330P</VersionCode>
					<StatisticalCode>XRPT03</StatisticalCode>
					<xsl:for-each select="$Dtm">
						<Authorisation>
							<xsl:call-template name="DTM203ToDateTime"/>
						</Authorisation>
					</xsl:for-each>
					<TypeCode>XRPT03</TypeCode>
					<!--
					<StatusCode>
				...
				<xsl:for-each select="$S06Patient">
					...
					<xsl:for-each select="$S07Patient">
						<xsl:for-each select="HAN">
							<Consent>
								<Given>true</Given>
								<xsl:call-template name="HANToConsentText"/>
							</Consent>
						</xsl:for-each>
					</xsl:for-each>
				</xsl:for-each>
			...
			<LaboratoryResults>
				...
				<CodedFormat>
					...
					<xsl:for-each select="$S16Samples">
						<Sample>
							...
							<xsl:for-each select="$S18Materials">
								<CodedResults>
				...
				<TextualFormat>
					<xsl:for-each select="$S18Macroscopic">
						<Macroscopic>
							<xsl:call-template name="Error">
								<xsl:with-param name="Text">Det er ikke tilladt at sende makroskopi i RPT03</xsl:with-param>
							</xsl:call-template>
						</Macroscopic>
					</xsl:for-each>
					...
					<xsl:for-each select="$S18Hematology">
						<xsl:call-template name="Error">
							<xsl:with-param name="Text">Det er ikke tilladt at sende hæmatologi i RPT03</xsl:with-param>
						</xsl:call-template>
					</xsl:for-each>
