	...
	<xsl:template match="m:DiabetesReport">
		...
		<Brev serializeable="no">
			<xsl:for-each select="m:Report">
				...
				<!-- Historisk må ikke være tomt -->
				<xsl:for-each select="m:History">
					<xsl:if test="count(*)=0">
						<xsl:call-template name="DRError">
							<xsl:with-param name="Text">History feltet må ikke være tomt</xsl:with-param> 
						</xsl:call-template>
					</xsl:if>
				</xsl:for-each>

				<!-- CurrentTreatment må ikke være tomt -->
				<xsl:for-each select="m:CurrentTreatment">
					<xsl:if test="count(*)=0">
						<xsl:call-template name="DRError">
							<xsl:with-param name="Text">CurrentTreatment feltet må ikke være tomt</xsl:with-param> 
						</xsl:call-template>
					</xsl:if>
				</xsl:for-each>
