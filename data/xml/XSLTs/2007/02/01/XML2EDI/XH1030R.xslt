...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/" version="1.0">
	...
	<xsl:template match="m:PsychologistReferral">
...
-->
							...
							<Elm>
								...
								<SubElm>
									...
									<xsl:choose>
										...
										<xsl:when test="$DTC='psykologhenvisning' ">HET</xsl:when>
										<xsl:when test="$DTC='Psykologhenvisningsdiagnose' ">SFU</xsl:when>										
										<xsl:otherwise>
								...
								<SubElm>
									<!-- oversæt diagnosetekst. Hvis der er tale om en psykologhenvisningdiagnose og diagnoseteksten i xml'en
										stemmer overens med diagnosekodens definitionstekst, medtages diagnoseteksten ikke i edi'en. TODO! -->
									<xsl:variable name="DTC" select="$Diagnose/m:DiagnoseTypeCode"/>
									<xsl:choose>
										<xsl:when test="$DTC='Psykologhenvisningsdiagnose' "> 
											<xsl:call-template name="createPsykologHenvisningDiagnoseText">
							...
							<Elm>
								...
								<SubElm>
									...
									<xsl:choose>
										...
										<xsl:when test="$Desc='speciallaegehenvisning'">HI</xsl:when> 
										<xsl:when test="$Desc='fysioterapeuthenvisning'">HI</xsl:when> 
										<xsl:when test="$Desc='psykologhenvisning'">HI</xsl:when> 
										<xsl:otherwise>
							...
							<Elm>
								...
								<SubElm>
									...
									<xsl:choose>
										...
										<xsl:when test="$DTC='uspecificeretkode' ">USP</xsl:when>
										<xsl:when test="$DTC='ICPCkode' ">ICP</xsl:when>
										<xsl:when test="$DTC='Speciallaegehenvisning' ">HET</xsl:when>										
	...
	<!-- Template til at skabe en diagnosetekst til en psykologhenvisningsdiagnose --> 
	<xsl:template name="createPsykologHenvisningDiagnoseText">
		<xsl:param name="DC"/>
		<xsl:param name="DT"/> 
		<xsl:choose>
			<!-- Hvis DC og DT matcher en kendt diagnosekode og dennes diagnosetekst, skal der ikke være nogen diagnosetekst i edi'en -->
			<xsl:when test=" $DC='1' and $DT='Røveri-, volds – eller voldtægtsoffer'"/>
			<xsl:when test=" $DC='2' and $DT='Trafik eller ulykkesoffer'"/>
			<xsl:when test=" $DC='3' and $DT='Pårørende til alvorligt psykisk syge personer'"/>
			<xsl:when test=" $DC='4' and $DT='Personer ramt af alvorligt invaliderende sygdom'"/>
			<xsl:when test=" $DC='5' and $DT='Pårørende til personer ramt af invaliderende sygdom'"/>
			<xsl:when test=" $DC='6' and $DT='Pårørende ved dødsfald'"/>
			<xsl:when test=" $DC='7' and $DT='Personer der har forsøgt selvmord'"/>
			<xsl:when test=" $DC='8' and $DT='Kvinder, der får foretaget provokeret abort efter 12. graviditetsuge.'"/>
			<xsl:when test=" $DC='9' and $DT='Personer der inden det fyldte 18. år har været ofre for incest.'"/>
			<xsl:when test=" $DC='10' and $DT='Personer med let til moderat depression (18-37 år).'"/>
			<xsl:when test=" $DC='11' and $DT='Personer med let til moderat angst, ocd (18-28 år)'"/>
			<xsl:when test=" $DC='99' and $DT='Fravigelse af 6 mdrs. fristen.'"/>
			<xsl:otherwise>
