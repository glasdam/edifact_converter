...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/" version="1.0">
	...
	<xsl:template match="m:TeddyReferral">
...
-->
	...
	<xsl:template name="createFysioterapiHenvisningDianoseText2">
		<xsl:param name="DC"/>
		<xsl:param name="DT"/> 
		<xsl:choose>
			...
			<xsl:when test=" $DC='1K' and $DT='Andre medfødte eller arvelige sygdomme med motoriske handicaps som fx adrenoleukodystrofi,
Spielmeyers-Vogts syndrom, familiær amyloidose'"/>
			...
			<xsl:when test=" $DC='1O' and $DT='Arvelige bindevævssygdomme som fx osteogenesis imperfecta, Marfans syndrom, Ehlers
syndrom, Morquios syndrom'"/>
			...
			<xsl:when test=" $DC='2A' and $DT='Følger efter hjerte-, rygmarvs- og hjernehindebetændelse (encephalitis, myelitis, meningitis)'"/>
			<xsl:when test=" $DC='2B' and $DT='Neurologiske kompl. til AIDS'"/>
			<xsl:when test=" $DC='2C' and $DT='Følger efter poliomyelitis'"/>
			<xsl:when test=" $DC='2D' and $DT='Følger efter hjerneblødning (apoplexia cerebri incl. subarachnoidalblødning)'"/>
			...
			<xsl:when test=" $DC='4D' and $DT='Kronisk polyarthritis fx som følge af LED, morbus Sjøgren, morbus Reiter'"/>
