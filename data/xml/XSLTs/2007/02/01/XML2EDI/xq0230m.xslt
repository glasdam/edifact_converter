...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://rep.oio.dk/sundcom.dk/medcom.dk/xml/schemas/2007/02/01/" version="1.0">
	...
	<!-- DISCHARGELETTER -->
	<xsl:template match="m:MicrobiologyRequest">
				...
				<xsl:for-each select="$ReqAnas">
					<S17 hidden="true">
						...
						<xsl:for-each select="m:Analysis">
							<INV>
								...
								<Elm>
									...
									<SubElm>
										...
										<xsl:choose>
											<xsl:when test="$ACT='iupac' ">CQU</xsl:when>
											<xsl:when test="$ACT='lokal' ">MDS</xsl:when>
											<xsl:otherwise>
												<FEJL>Ukendt  AnalysisCodeType: <xsl:value-of select="$ACT"/>
												</FEJL>
											</xsl:otherwise>
							...
							<xsl:for-each select="m:AnalysisMDSName">
								<FTX>
									<Elm>
										<SubElm>ACM</SubElm>
									</Elm>
						...
						<xsl:for-each select="m:SupplementaryLocation">
							<FTX>
								<Elm>
									<SubElm>SRC</SubElm>
								</Elm>
						...
						<xsl:for-each select="m:SupplementaryAnalysis">
							<FTX>
								<Elm>
									<SubElm>IVC</SubElm>
								</Elm>
